...
const VerificationChangePassword = ({ navigation,route }) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      console.log("try")
      const response = await fetch(`${API_BASE_URL}/verify1`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        Alert.alert('Success', 'Registration successful!', [
          { text: 'OK', onPress: ()=>navigation.navigate("ChangePassword",{verification_code:code,phoneNumber:phoneNumber})},
        ]);
      } else {
...
export default VerificationChangePassword;
